...
### 多阶段关联评分
推荐系统采用**序列化选品**策略，每选一件衣物都会考虑已选单品的协调度：

| 评分维度 | 权重 | 说明 |
|----------|------|------|
| 颜色和谐 | 30% | 36组经典配色对，3级评分 |
| 风格一致 | 25% | Jaccard 相似度计算 |
| 正式度协调 | 20% | 标准差越小越好 |
| 场合匹配 | 15% | 数据驱动评分矩阵 |
| 用户偏好 | 10% | 风格测试 + 学习权重 |
...
### AI 穿搭助手
- iOS 26+ 接入 Apple Foundation Models（设备端大模型）
- 旧版本自动降级为规则引擎
- 支持自然语言交互

### 真实天气
接入 [Open-Meteo](https://open-meteo.com) 免费天气 API，基于 GPS 定位自动获取。

## 后续优化计划

- [x] ~~接入真实天气 API~~ ✅ Open-Meteo
- [x] ~~集成 AI 大模型~~ ✅ Apple Foundation Models
- [x] ~~搭配协调打分~~ ✅ 多阶段关联评分
- [x] ~~学习能力~~ ✅ 统计学习服务
